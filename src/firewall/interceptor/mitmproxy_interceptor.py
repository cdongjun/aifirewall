...
class MitmproxyInterceptor:
    def __init__(
        ...
        self.block_image_bytes = requests.get("https://http.cat/403", proxies={"http":None, "https":None}).content


    async def request(self, flow: http.HTTPFlow):
        http_log = parse_http_request(flow)
        if not flow.request.url.endswith([".png",".jpng"]):
    ...
    async def response(self, flow: http.HTTPFlow):
        ...
        for policy_name, policy_info in self.policies["http"].items():
            if matches_policy(flow,policy_info):
                if content_type.startswith("image/jpeg") or content_type.startswith("image/jpeg"):
